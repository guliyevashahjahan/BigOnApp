...
namespace BigOn.Infrastructure.Services.Concrates
{
    public class CryptoService : ICryptoService
    {
        ...
        public string ToMd5(string text)
        {
            ...
            {
                byte[] buffer = Encoding.UTF8.GetBytes($"{options.SaltKey}!{text}#{string.Join("", options.SaltKey.ToCharArray().Reverse())}");

                byte[] result = provider.ComputeHash(buffer);

                return string.Join("", result.Select(m => m.ToString("x2")));
            }
        }
